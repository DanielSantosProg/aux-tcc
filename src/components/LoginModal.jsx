...
const LoginModal = ({data, login}) => {
    ...
    const handleSubmit = () => {
        ...
        if (user) {
          login(user); // chama o contexto de login
          setErro(null);
        } else {
                ...
                <div class="relative bg-white rounded-lg shadow-sm">
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit}>
                            <div>
                                ...
                                <input type="email" name="email" id="email" onChange={(e) => setEmail(e.target.value)} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="nome@ifba.edu.br" required />
                            </div>
                            <div>
                                <label for="password" class="block mb-2 text-sm font-medium text-gray-900">Digite sua senha</label>
                                <input type="password" name="password" id="password" placeholder="••••••••" onChange={(e) => setPassword(e.target.value)} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
                            </div>
                            ...
                            <div class="flex justify-between">
                                <div class="flex items-start">
                                    ...
                                    <label for="remember" class="ms-2 text-sm font-medium text-gray-900">Lembre-se de mim</label>
                                </div>
